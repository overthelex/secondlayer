...
describe('Specialized hooks', () => {
  ...
  it('useGetLegalAdvice should use correct tool', async () => {
    const { useGetLegalAdvice } = await import('../useMCPTool');
    const mockController = new AbortController();
    (mcpService.streamTool as any).mockResolvedValue(mockController);

    const { result } = renderHook(() => useGetLegalAdvice());
  ...
  it('useSearchCourtCases should use correct tool', async () => {
    const { useSearchCourtCases } = await import('../useMCPTool');
    const mockController = new AbortController();
    (mcpService.streamTool as any).mockResolvedValue(mockController);

    const { result } = renderHook(() => useSearchCourtCases());

    await act(async () => {
      await result.current.executeTool({ query: 'Test' });
    });

    expect(useChatStore.getState().currentTool).toBe('search_court_cases');
  });
});
