/**
 * RADA MCP Server - HTTP REST entry point
 * Provides parliament data analysis via REST API with SSE streaming
 */
...
class HTTPRadaServer {
  ...
  async start() {
    ...
    this.app.listen(port, host, () => {
      logger.info(`HTTP RADA Server started on http://${host}:${port}`);
...
// Start server
const server = new HTTPRadaServer();
server.start().catch((error) => {
  logger.error('Failed to start HTTP RADA server:', error);
  process.exit(1);
});
