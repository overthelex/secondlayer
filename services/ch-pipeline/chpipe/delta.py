...
from .portals import OFF_LISTING_SPIDERS
...
def court_code_spider_map(conn) -> dict[str, tuple[str, ...]]:
    ...
    for row in rows:
        ...
        if spider in OFF_LISTING_SPIDERS:
            # A regulator or court that publishes on its own site (chpipe/portals,
            # walked by run-portals.sh) or the RPW journal cut (rpw_stage): never
            # from an entscheidsuche listing, so it has no place in this map and
            # is not a misconfiguration worth a warning.
            continue
