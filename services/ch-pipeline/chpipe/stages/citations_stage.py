...
a decision's text has been scanned SUCCESSFULLY. A decision whose extraction
raises is deliberately NOT stamped and NOT touched: its old edges stay where
they are and the next run tries again. The exception is logged with the
decision's ecli and counted in `failed`; last_error is left alone -- this
stage does not own that column (db.fail()/db.mark_failed() do, for the
stage-column queue).

Leaving a failure unstamped means the claim query keeps offering it, so this
run remembers the eclis that raised and skips them in later batches: a batch
that is nothing but rows this run already failed ends the run with a warning
naming the count. That is the whole retry policy -- there is no attempt
counter. A text that raises every night shows up in every night's log as
`failed=N` plus that warning, which is the signal to go and look at it; it
costs one extraction attempt per run and never blocks the decisions behind
it, because the skip keeps the queue moving within the run.
...
the queue is a flag column, so a smaller batch costs nothing but extra
round-trips.
"""
            ...
            while True:
                ...
                for future in concurrent.futures.as_completed(futures):
                    ...
                    try:
                        ...
                        # gets to try again on the same text.
