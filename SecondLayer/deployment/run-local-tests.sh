...
# Run tests locally (on host machine, not in container)
run_tests_locally() {
  local project_dir=$1
  local test_path=$2
  local test_name=$3
...
if [ "$RADA_ONLY" = false ]; then
  ...
  if [ "$QUICK_MODE" = true ]; then
    ...
    if run_tests_locally "mcp_backend" "smoke-test-all-tools.test.ts" "Backend Smoke Tests"; then
    ...
    # Integration tests
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_backend" "all-tools-integration.test.ts" "Backend Integration Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
    ...
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_backend" "document-analysis-e2e.test.ts" "Document Service E2E Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
    ...
    # Due diligence tools
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_backend" "due-diligence-tools.test.ts" "Due Diligence Tools Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
    ...
    # Legal advice tests
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_backend" "get-legal-advice-cpc-gpc.test.ts" "Legal Advice Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
    ...
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_backend" "search-legal-precedents.test.ts" "Legal Precedents Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
...
if [ "$RADA_RUNNING" = true ] && [ "$BACKEND_ONLY" = false ]; then
  ...
  if [ "$QUICK_MODE" = true ]; then
    # Quick mode - only smoke tests
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_rada" "smoke-test-rada-tools.test.ts" "RADA Smoke Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
    ...
    TOTAL_TESTS=$((TOTAL_TESTS + 1))
    if run_tests_locally "mcp_rada" "all-rada-tools-integration.test.ts" "RADA Integration Tests"; then
      PASSED_TESTS=$((PASSED_TESTS + 1))
    else
      FAILED_TESTS=$((FAILED_TESTS + 1))
    fi
