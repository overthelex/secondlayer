...
import { getTemplateClassifier, TemplateMatcher, TemplateGenerator, TemplateStorage, TemplateVersionManager } from '../services/template-system/index.js';
...
export function createTemplateRoutes(db: BaseDatabase): Router {
   ...
   * Get all versions with metrics for a template
   */
  router.get('/:id/versions', requireAuth, async (req: Request, res: Response) => {
    try {
      const templateId = req.params.id as string;
      const { userSegment } = req.query;

      const versionManager = new TemplateVersionManager(db);
      const versions = await versionManager.getVersionsWithMetrics(
        templateId,
        userSegment as string | undefined
      );
  ...
  /**
   * GET /api/templates/:id/best-version
   * Get best version for user segment
   */
  router.get('/:id/best-version', requireAuth, async (req: Request, res: Response) => {
    try {
      const templateId = req.params.id as string;
      const { userSegment = 'general' } = req.query;

      const versionManager = new TemplateVersionManager(db);
      const bestVersion = await versionManager.getBestVersionForSegment(
        templateId,
      ...
      if (!bestVersion) {
        return res.status(404).json({
          error: 'No released version found for this segment',
        });
      }
      ...
      logger.error('Failed to get best version', {
        error: (error as Error).message,
      });
      return res.status(500).json({
        error: 'Failed to retrieve best version',
      });
    }
  });

  /**
   * GET /api/templates/:id/versions/:versionId/eligibility
   * Check if version is eligible for promotion
   */
  router.get('/:id/versions/:versionId/eligibility', requireAdminAuth, async (req: Request, res: Response) => {
    try {
      const versionId = req.params.versionId as string;

      const versionManager = new TemplateVersionManager(db);
      const eligibility = await versionManager.checkPromotionEligibility(versionId);
   ...
   */
  router.post(
    '/:id/versions/:versionId/promote',
    requireAdminAuth,
    async (req: Request, res: Response) => {
      try {
        const templateId = req.params.id as string;
        const versionId = req.params.versionId as string;
        const { toStatus, userSegment = 'general', reason } = req.body;
        const authReq = req as AuthenticatedRequest;

        if (!toStatus || !['candidate', 'released', 'deprecated'].includes(toStatus)) {
          return res.status(400).json({
            error: 'Invalid toStatus. Must be: candidate, released, or deprecated',
          });
        }
        ...
        const versionResult = await db.query(
          `SELECT status FROM template_versions WHERE id = $1`,
          [versionId]
        );
        ...
        const fromStatus = versionResult.rows[0].status;

        await versionManager.promoteVersion({
          templateId,
          versionId,
          userSegment,
          fromStatus,
          toStatus,
          promotedBy: authReq.user?.id || 'admin',
          reason,
        });
        ...
        return res.status(200).json({
          ...
          toStatus,
          userSegment,
          message: `Version promoted from ${fromStatus} to ${toStatus}`,
        });
      } catch (error) {
  ...
  /**
   * GET /api/templates/:id/promotion-history
   * Get promotion history for a template
   */
  router.get('/:id/promotion-history', requireAuth, async (req: Request, res: Response) => {
    try {
      const templateId = req.params.id as string;
      ...
      const versionManager = new TemplateVersionManager(db);
      const history = await versionManager.getPromotionHistory(templateId, limitNum);
  ...
  /**
   * POST /api/admin/auto-promotion
   * Run auto-promotion job (admin only)
   * Promotes candidates to released based on metrics
   */
  router.post('/admin/auto-promotion', requireAdminAuth, async (req: Request, res: Response) => {
    try {
      const versionManager = new TemplateVersionManager(db);
      const result = await versionManager.runAutoPromotionJob();

      return res.status(200).json({
        message: 'Auto-promotion job completed',
      ...
      return res.status(500).json({
        error: 'Auto-promotion job failed',
      });
    }
  });

  /**
   * POST /api/admin/update-promotion-scores
   * Update promotion scores for all versions (admin only)
   * Call this daily for accurate promotion readiness
   */
  router.post(
    '/admin/update-promotion-scores',
    requireAdminAuth,
    async (req: Request, res: Response) => {
      try {
        const versionManager = new TemplateVersionManager(db);
        const result = await versionManager.updatePromotionScores();

        return res.status(200).json({
          message: 'Promotion scores updated',
          updated: result.updated,
        });
      } catch (error) {
        logger.error('Update promotion scores failed', {
          error: (error as Error).message,
        });
        return res.status(500).json({
          error: 'Failed to update promotion scores',
        });
      }
    }
  );
