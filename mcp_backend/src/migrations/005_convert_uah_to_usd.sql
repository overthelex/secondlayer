...
BEGIN
  ...
  -- Step 2: Convert existing UAH values to USD only if UAH column exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'cost_tracking' AND column_name = 'zakononline_cost_uah'
  ) THEN
    EXECUTE 'UPDATE cost_tracking
    ...
    ALTER TABLE cost_tracking DROP COLUMN zakononline_cost_uah;
  END IF;

  -- Step 4: Drop total_cost_uah column (we only use total_cost_usd now)
  ALTER TABLE cost_tracking DROP COLUMN IF EXISTS total_cost_uah;
  ...
  -- Step 6: Convert existing monthly stats from UAH to USD only if UAH column exists
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'monthly_api_usage' AND column_name = 'zakononline_total_cost_uah'
  ) THEN
    EXECUTE 'UPDATE monthly_api_usage
    ...
    ALTER TABLE monthly_api_usage DROP COLUMN zakononline_total_cost_uah;
  END IF;
END $$;
