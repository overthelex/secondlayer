...
-- This migration adds tables for tracking API costs (OpenAI, Anthropic, Database queries)
...
CREATE TABLE IF NOT EXISTS cost_tracking (
  ...
  -- Database queries (FREE but track for monitoring)
  database_queries_count INTEGER DEFAULT 0,
  database_total_rows INTEGER DEFAULT 0,
  database_calls JSONB DEFAULT '[]', -- [{query_type, execution_time_ms, rows_returned, timestamp}]

  -- SecondLayer API calls (for cross-referencing with court cases)
...
CREATE TABLE IF NOT EXISTS monthly_api_usage (
  ...
  -- Database queries (free but track usage)
  database_total_queries INTEGER DEFAULT 0,
  database_total_rows BIGINT DEFAULT 0,
