...
export class StreamingXMLParser {
    ...
    return new Promise((resolve, reject) => {
      ...
      parser.on('closetag', (tagName) => {
        if (tagName === 'SUBJECT' && currentSubject) {
          ...
          // Store value in object or subject
  ...
  private extractUOData(subject: any): ParsedUOEntity {
    ...
    if (subject.EXECUTIVE_POWER) {
      entity.executive_power = subject.EXECUTIVE_POWER;
    }

    if (subject.FOUNDERS?.FOUNDER) {
      entity.founders = subject.FOUNDERS.FOUNDER;
    }

    if (subject.BENEFICIARIES?.BENEFICIARY) {
      entity.beneficiaries = subject.BENEFICIARIES.BENEFICIARY;
    }

    if (subject.SIGNERS?.SIGNER) {
      entity.signers = subject.SIGNERS.SIGNER;
    }

    if (subject.MEMBERS?.MEMBER) {
      entity.members = subject.MEMBERS.MEMBER;
    }

    if (subject.BRANCHES?.BRANCH) {
      entity.branches = subject.BRANCHES.BRANCH;
    }

    if (subject.PREDECESSORS?.PREDECESSOR) {
      entity.predecessors = subject.PREDECESSORS.PREDECESSOR;
    }

    if (subject.ASSIGNEES?.ASSIGNEE) {
      entity.assignees = subject.ASSIGNEES.ASSIGNEE;
    }

    if (subject.TERMINATION_STARTED_INFO) {
      entity.termination_started = subject.TERMINATION_STARTED_INFO;
    }

    if (subject.BANKRUPTCY_READJUSTMENT_INFO) {
      entity.bankruptcy_info = subject.BANKRUPTCY_READJUSTMENT_INFO;
    }

    if (subject.EXCHANGE_DATA?.EXCHANGE_ANSWER) {
      entity.exchange_data = subject.EXCHANGE_DATA.EXCHANGE_ANSWER;
    }
