 ...
 * Usage: node dist/scripts/import-uo-parallel.js <zipPath> [workers=10]
 */
import { Pool } from 'pg';
  ...
  try {
    ...
    const count = await parser.parseBatched<ParsedUOEntity>(
      ...
      async (entities) => {
        ...
        // Launch worker — don't await, let it run in parallel
        const p = (async () => {
          try {
            await importer.importUOEntities(entities, entities.length);
            totalImported += entities.length;
            if (totalImported % 10000 < BATCH_SIZE) {
              const elapsed = ((Date.now() - startTime) / 1000).toFixed(0);
              console.log(`  Imported ${totalImported} entities (${elapsed}s, ${totalErrors} errors, ${sem.pending} queued)`);
            }
          } catch (err) {
    ...
    // Wait for all inflight workers to finish
