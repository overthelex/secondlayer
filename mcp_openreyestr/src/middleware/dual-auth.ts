/**
 * Dual Authentication Middleware for OpenReyestr
...
const OPENREYESTR_API_KEYS = process.env.OPENREYESTR_API_KEYS?.split(',').map(k => k.trim()).filter(k => k) || [];
...
function authenticateWithAPIKey(req: AuthenticatedRequest, apiKey: string): void {
  ...
  if (!OPENREYESTR_API_KEYS.includes(apiKey)) {
